...
accepted; amended 2026-05-17 to make shell visual artifacts optional by architecture-review-r1

## Context

The shell visual-coherence follow-on spec extends the first UI design-system slice from file-list resources into app-wide shell surfaces. The change affects multiple long-lived architecture surfaces: token and scope contract evolution, fixture icon resources, static validation, optional full-shell screenshot artifacts, high-DPI behavior checks, sidebar behavior preservation, and the boundary between visual fixture proof and real V1 behavior proof.
...
accepted shell visual-coherence spec
  ...
  -> optional full-shell soft-review visual artifacts
...
Optional full-shell visual artifacts may use effective-pixel app-window profiles:
...
`shell-standard-1440x900-200` is useful for high-DPI review when optional artifacts are recorded, but it is not required milestone or final-closeout evidence. Screenshot evidence remains optional soft-review context; hard pixel or perceptual comparison requires a later accepted spec and architecture decision.
...
- Keep icon fixture rendering as extension text chips: rejected because placeholder-looking text chips undermine deterministic review artifacts and repeat the defect class exposed by the earlier glyph regression.
...
- Require only the standard `1440 x 900` profile: rejected because this ADR no longer requires visual profiles as closeout gates; optional profiles should be chosen based on the review question.
...
- UI contract tooling expands from first-slice tokens and file-list scopes into shell-wide scopes, icon-resource invariants, and optional screenshot sidecar validation.
...
- Full-shell screenshots may be recorded as optional review artifacts across chosen shell states and effective-pixel profiles.
- High-DPI review remains explicit through tests/manual notes when a touched region creates a high-DPI risk; screenshots are optional support.
...
- The matching test spec must map shell visual-coherence requirements to static UI contract tests, icon invariant tests, optional screenshot sidecar guardrails, accessibility checks, and behavior-preservation matrix rows.
