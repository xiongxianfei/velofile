...
- Goal: Promote the M2 validation foundations into benchmark reporting, accessibility evidence, diagnostics conformance gates, compatibility aggregation, and preview-release triage threshold support without overstating infrastructure checks as product release evidence.
...
- Validation commands:
  ...
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/ci.ps1`
- Expected observable result: Release readiness can be judged using generated corpus, benchmark reports, diagnostics thresholds, and compatibility checks, with missing app-level or verifier evidence reported as incomplete rather than verified.
...
- Milestone closeout:
  ...
  - review-resolution decision: current benchmark scenarios are classified as `infrastructure-only` with `releaseEvidence=false` until an app-level driver exists.
  - review-resolution decision: `compat --scope release` consumes scope result documents and returns incomplete/nonzero when required association, DPI, or behavior-verifier evidence is missing.
...
- M15 review-resolution evidence:
  - Added tests proving benchmark measurements cannot satisfy P1-P13/AC15 unless they are app-level, app-boundary-driven release evidence.
  - Added tests proving release compatibility aggregation does not upgrade fixture-only operations/drag-drop results, skipped path cases, or missing association/DPI verifiers to verified evidence.
  - Added diagnostics conformance coverage for navigation, preview, file operation, search, terminal, and session-restore workflows plus below/at/above threshold release decisions.
  - Added `docs/release/accessibility-checklist.md` and tests requiring checklist coverage for keyboard traversal, focus visibility, destructive confirmation readability, distinct states, screen-reader metadata, and mixed-DPI checks.
  - `dotnet test tests\VeloFile.Corpus.Tests\VeloFile.Corpus.Tests.csproj -c Debug --filter "Compatibility_release_scope_consumes_real_scope_results_without_upgrading_missing_evidence|Benchmark_harness_emits_measured_report_environment_and_release_status|Diagnostics_conformance_runner_writes_redacted_local_report_and_export"` passed 3 Corpus tests.
  - `dotnet test tests\VeloFile.App.Tests\VeloFile.App.Tests.csproj -c Debug --filter "M15_accessibility_release_evidence_requires_manual_checklist_for_focus_keyboard_and_mixed_dpi"` passed 1 App test.
  - `dotnet test tests\VeloFile.App.Tests\VeloFile.App.Tests.csproj -c Debug --filter Accessibility` passed 3 App accessibility contract tests.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-benchmarks.ps1 -NonGating -ScratchRoot <scratch-root> -RunCount 2` passed and wrote infrastructure-only benchmark evidence.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-diagnostics-conformance.ps1 -ScratchRoot <scratch-root>` passed and wrote all workflow/triage conformance evidence.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-compat-corpus.ps1 -Scope release -ScratchRoot <scratch-root>` intentionally returned 1 and wrote `status=incomplete` because release verifier evidence is missing.
  - `dotnet test tests\VeloFile.Corpus.Tests\VeloFile.Corpus.Tests.csproj -c Debug` passed 13 Corpus tests after increasing the validation timeout to cover the full script-heavy suite.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\ci.ps1` passed `dotnet --info`, restore, build with 0 warnings and 0 errors, and 330 tests across 4 test assemblies.
- M16 test-first implementation evidence:
  - Added App release packaging contract tests before the MSIX manifest, package script, release verifier, release docs, user differences docs, and README links existed; the first focused run failed on the missing M16 artifacts.
  - Added `Package.appxmanifest`, Release publish/package properties, `scripts\package-msix.ps1`, product-specific `scripts\release-verify.ps1`, a PowerShell-backed `release-verify.sh`, and a Windows release workflow that builds a signed release package when a certificate thumbprint is supplied.
  - Updated the package script after validation proved metadata-only release artifacts were too weak; unsigned local runs now create an actual `.msix` with Windows SDK `MakeAppx`, and signed release runs additionally require `SignTool` and a signing thumbprint.
  - Added stable update channel, install/rollback/uninstall, V1 release notes, release checklist, and Differences from File Explorer docs covering default extension display, `invoice.pdf.exe`, per-application extension settings, built-in context menu scope, and Explorer/file-association non-ownership.
  - Updated README release links, SECURITY release-integrity guidance, and `.gitignore` for generated packaging artifacts.
- M16 validation:
  - `dotnet test tests\VeloFile.App.Tests\VeloFile.App.Tests.csproj -c Debug --filter ReleasePackagingContractTests` passed 4 App release packaging contract tests.
  - `dotnet publish src\VeloFile.App\VeloFile.App.csproj -c Release` passed and produced the Release publish layout.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\package-msix.ps1` passed and produced an unsigned local MSIX plus package metadata under ignored `artifacts\msix`.
  - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\release-verify.ps1` passed and exercised publish, MSIX package creation, manifest checks, and release documentation checks.
