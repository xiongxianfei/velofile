...
- M12 test-first implementation evidence:
  ...
  - Image preview now supports common image extensions by bounded header inspection, enforces the 100 MB input cap, and rejects decoded dimensions over 8192 by 8192.
  ...
  - PDF preview now validates a bounded first-page-capable PDF sample, returns first-page preview content, and uses the 500 MB metadata-only cap.
  - Production app composition now injects the Windows provider chain before metadata fallback.
  - The preview corpus `providers` scope now writes behavior-verifier evidence for image success, text truncation, PDF first-page, over-size fallback, and source non-mutation.
...
- M12 validation:
  - `dotnet test tests\VeloFile.Windows.Tests\VeloFile.Windows.Tests.csproj -c Debug --filter PreviewProviders` passed: 5 Windows provider tests.
  ...
  - `dotnet test VeloFile.sln -c Debug --filter PreviewProviders` passed: 1 App, 5 Windows, and 1 Corpus provider tests; Core tests had no matching filter.
  - `dotnet test VeloFile.sln -c Debug --filter PreviewContract` passed: 17 Core, 4 App, and 1 Corpus preview-contract tests; Windows tests had no matching filter.
  - `dotnet build VeloFile.sln -c Debug` passed with 0 warnings and 0 errors.
